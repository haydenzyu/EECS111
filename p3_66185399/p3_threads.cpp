...
void *threadfunc(void *param)
{
	...
	while (true) {
		...
		if (global_work==0) {//if main locks
			pthread_mutex_unlock(&mutex);//critical section ends
			break;
		}
		// push id
		readyQue.push_back(id); //push tcb[]->id to readyQue
		...
		while (true){
		pthread_cond_wait(&(cond[id]), &mutex); //wait for cond[i] from main
		if (occupied==0) break; 
		}
		
		if (global_work==0) { //if main not working
			pthread_mutex_unlock(&mutex); //critical section ends
			break;
		}
		
		occupied = 1; //this thread is working
		pthread_mutex_unlock(&mutex); //critical section ends
